...
def test_step_reward_calculation(rl_env):
    ...
    # Total revenue is now 20.0 (Delta = 10.0)
    rl_env.stats["captured_revenue"] = [10.0, 10.0]

    # Total waste is 1 Burger (2.0) + 1 Fries (1.0) = 3.0 (Delta = 1.0)
    rl_env.stats["wasted_burgers"] = [1]
    rl_env.stats["wasted_fries"] = [1]

    # Total lost revenue is 5.0 (Delta = 5.0)
    ...
    # 4. Verify Math:
    # delta_rev (10.0) - delta_waste (1.0) - delta_lost (5.0) = 4.0
    # Scaled by 10.0 -> Reward = 0.4
    assert reward == 0.4
